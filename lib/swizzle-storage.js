...
exports.deleteFile = exports.saveFile = exports.getFileUrl = exports.removeUserFromFile = exports.addUserToFile = exports.storageHandler = void 0;
const storage_1 = require("@google-cloud/storage");
const mongodb_1 = require("mongodb");
const _1 = require(".");
const storage = new storage_1.Storage(); //Check on this - is this working in digital ocean?
//URL to access files
...
const storageHandler = async (request, result) => {
    try {
        const fileName = request.params.key;
        const lastIndex = fileName.lastIndexOf('.');
        const nameWithoutExtension = lastIndex !== -1 ? fileName.substring(0, lastIndex) : fileName;
        const document = await _1.db.collection('_swizzle_storage').findOne({ _id: new mongodb_1.ObjectId(nameWithoutExtension) });
        if (!document) {
...
*/
async function addUserToFile(unsignedFileUrl, userId) {
    try {
        ...
        var document = await _1.db.collection('_swizzle_storage').updateOne({ _id: new mongodb_1.ObjectId(nameWithoutExtension) }, { $addToSet: { allowedUsers: stringUserId } });
...
Returns a success boolean.
*/
async function removeUserFromFile(unsignedFileUrl, userId) {
    try {
        var stringUserId = userId;
        if (userId instanceof mongodb_1.ObjectId) {
            stringUserId = userId.toHexString();
        ...
        var document = await _1.db.collection('_swizzle_storage').updateOne({ _id: new mongodb_1.ObjectId(nameWithoutExtension) }, { $pull: { allowedUsers: userId } });
        if (document.modifiedCount === 0) {
            return false;
        }
        return true;
...
exports.removeUserFromFile = removeUserFromFile;
/*
usage:
...
*/
async function getFileUrl(unsignedFileUrl, fileName) {
    var document = null;
    if (unsignedFileUrl) {
        const fileName = unsignedFileUrl.substring(unsignedFileUrl.lastIndexOf('/') + 1);
        var nameWithoutExtension = fileName.substring(0, fileName.lastIndexOf('.'));
        ...
        document = await _1.db.collection('_swizzle_storage').findOne({ _id: new mongodb_1.ObjectId(nameWithoutExtension) });
    }
    else if (fileName) {
        var nameWithoutExtension = fileName.substring(0, fileName.lastIndexOf('.'));
        const isMongoObjectId = mongodb_1.ObjectId.isValid(nameWithoutExtension);
        if (isMongoObjectId) {
            document = await _1.db.collection('_swizzle_storage').findOne({ _id: new mongodb_1.ObjectId(nameWithoutExtension) });
        }
        if (!document) {
            document = await _1.db.collection('_swizzle_storage').findOne({ fileName: fileName });
        }
    }
    if (!document) {
        return null;
...
async function saveFile(fileName, fileData, isPrivate = false, allowedUsers = []) {
    try {
        ...
        const result = await _1.db.collection('_swizzle_storage').insertOne(document);
...
*/
async function deleteFile(unsignedFileUrl, fileName) {
    try {
        var document = null;
        if (unsignedFileUrl) {
            const fileName = unsignedFileUrl.substring(unsignedFileUrl.lastIndexOf('/') + 1);
        ...
        await _1.db.collection("_swizzle_storage").deleteOne({ _id: document._id });
